...
ScaleImage
 and 
RotateImage
  However, using this command it is possible to do both operations simultaneously.  It is also possible to shear images using this command.
...
See also: ScaleImage, RotateImage.
...
Graphics 600,400,16; Create an image that we can manipulate
...
Color 0,0,255 : Rect 0,25,25,25,1; Set up drawing & timing stuff

SetBuffer BackBuffer() 

timer=CreateTimer(30)

Repeat	; deal with keyboard inputs
	...
	If KeyHit(1) Then Exit; draw the screen
...
Function RunDemo()
	...
	Repeat
		...
		a#=scale#:b#=0:c#=0:d#=scale# ; reset the matrix; increase the degrees (or reset if >360)

		If degrees<360 Then degrees=degrees+4 Else degrees=0; and set up the matrix multipliers.  The explanation as to why these work can be found in most good; mathematics text books, and is too involved to go into here!
		...
		md#=Cos(degrees) ; multiply the two matrices.  
		...
		a#=na#:b#=nb#:c#=nc#:d#=nd#; handle input and draw the screen
...
End Function; Draw stuff on the screen, including transforming the image
...
Function DrawScreen(demo=False)
	...
	Text 10,110,"Current Parameters:"; ######################## This is where we actually use TFormImage using the input parameters
